...
class ExecuteShellCommand(Tool):
    ...
    description = (
        ...
        f"Output is truncated at {_MAX_OUTPUT} chars; timeout defaults to {_MAX_TIMEOUT}s."
    )
    # "Prefer read_file/write_file/edit_file over cat/echo/sed, "

    def __init__(self):
        workspace_config = get_workspace_config()
        self.working_dir = workspace_config.workspace
        self.path_append = workspace_config.python
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        # on cwd.
        if working_dir and self.restrict_to_workspace and self.working_dir:
            try:
        ...
        result = await ProcessManager().exec(command, cwd, env, effective_timeout)
        return str(result)
...
class ProcessResult:
    ...
    def __str__(self):
        ...
        if self.out:
            output_parts.append(self.out)

        if self.err:
            stderr_text = self.err
