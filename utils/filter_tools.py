...
def filter_tool(tools_manager: ToolsManager, prompt: str, count=5):
    ...
    for tool in tools_manager.tools:
        tool_text = "Tool Name: {name}\nDescription: {description}".format(name=tool.name, description=tool.description)
    ...
    result = sorted(list(zip(sims, range(len(tools_manager.tools)))), reverse=True)[:count]
